  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        let ready = productsArray.filter((p: any) => p.isReadyToShip && p.name && p.name !== 'undefined');

        if (ready.length === 0) {
          ready = productsArray.filter((p: any) => p && p.isNew).slice(0, 20);
        }

        setProducts(ready.slice(0, 20));
      } catch (error) {
    ...
    <section className="section-padding bg-white">
      <div className="container-custom">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-gold text-sm font-medium tracking-wider uppercase block mb-1">Fast Delivery</span>
            <h2 className="font-playfair text-3xl md:text-4xl font-semibold text-black">Ready To Ship</h2>
          </div>
          <button
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Loading ready to ship products...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600">
              No ready to ship products found yet. Please add products in Admin → Ready To Ship.
            </p>
          </div>
        ) : (
