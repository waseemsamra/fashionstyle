...
export default function CelebrityClosetCMS() {
  ...
  const loadProducts = async () => {
    try {
      ...
      const collectionProducts = productsArray.filter((p: any) => p.isCelebrityCloset);
  ...
  const toggleProduct = (productId: string) => {
    setSelectedIds(prev => {
      ...
      if (prev.length >= MAX_CELEBRITY) { alert(`You can only select up to ${MAX_CELEBRITY} products`); return prev; }
  ...
  const handleSave = async () => {
    try {
      ...
      const updatePromises = allProducts.map(async (product) => {
        ...
        if (product.isCelebrityCloset !== shouldFlag) {
          return fetch(`${ADMIN_API_URL}/products/${product.id}`, {
            ...
            body: JSON.stringify({ ...product, isCelebrityCloset: shouldFlag })
          });
        }
      }).filter(Boolean);
    ...
    <div className="min-h-screen bg-beige-100 py-12">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-3xl font-bold mb-2">Celebrity Closet</h1><p className="text-gray-600">Select up to {MAX_CELEBRITY} products ({allProducts.length} total)</p></div>
          <div className="text-right"><p className="text-sm text-gray-600">Selected</p><p className={`text-2xl font-bold ${selectedIds.length >= MAX_CELEBRITY ? 'text-red-600' : 'text-gold'}`}>{selectedIds.length} / {MAX_CELEBRITY}</p></div>
