...
export default function CelebrityCloset() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        let celebrity = productsArray.filter((p: any) => p.isCelebrityCloset && p.name && p.name !== 'undefined');

        if (celebrity.length === 0) {
          celebrity = productsArray.filter((p: any) => p && p.isNew).slice(0, 20);
        }

        setProducts(celebrity.slice(0, 20));
      } catch (error) {
        console.error('Error loading celebrity closet:', error);
        setProducts([]);
      } finally {
        setIsLoading(false);
      }
    ...
    <section className="section-padding bg-white">
      <div className="container-custom">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-gold text-sm font-medium tracking-wider uppercase block mb-1">Glam</span>
            <h2 className="font-playfair text-3xl md:text-4xl font-semibold text-black">Celebrity Closet</h2>
          </div>
          <button
            onClick={() => navigate('/celebrity-closet')}
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Loading celebrity closet...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600">
              No celebrity closet products found yet. Please add products in Admin → Celebrity Closet.
            </p>
          </div>
        ) : (
