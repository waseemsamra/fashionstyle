...
export default function CelebrityClosetCMS() {
...
const handleSave = async () => { try { setSaving(true); localStorage.setItem('celebrityClosetProducts', JSON.stringify(selectedProductIds)); const token = localStorage.getItem('jwt_token'); if (!token) { alert('✅ Saved locally!'); return; } await Promise.all(products.map(async (product: any) => { const shouldFlag = selectedProductIds.includes(product.id); if (product.isCelebrityCloset !== shouldFlag) return fetch(`${ADMIN_API_URL}/products/${product.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token.replace(/^["']|["']$/g, '')}` }, body: JSON.stringify({ ...product, isCelebrityCloset: shouldFlag }) }); })); alert(`✅ Saved ${selectedProductIds.length} products!`); } catch (error) { alert('Failed to save'); } finally { setSaving(false); } };
