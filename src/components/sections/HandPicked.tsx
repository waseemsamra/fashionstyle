  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        let handpicked = productsArray.filter((p: any) => p.isHandPicked && p.name && p.name !== 'undefined');

        if (handpicked.length === 0) {
          handpicked = productsArray.filter((p: any) => p && p.isNew).slice(0, 20);
        }

        setProducts(handpicked.slice(0, 20));
      } catch (error) {
        console.error('Error loading hand picked:', error);
        setProducts([]);
      } finally {
        setIsLoading(false);
      }
    ...
    <section className="section-padding bg-white">
      <div className="container-custom">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-gold text-sm font-medium tracking-wider uppercase block mb-1">Curated</span>
            <h2 className="font-playfair text-3xl md:text-4xl font-semibold text-black">Hand Picked</h2>
          </div>
          <button
            onClick={() => navigate('/hand-picked')}
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Loading hand picked products...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600">
              No hand picked products found yet. Please add products in Admin → Hand Picked.
            </p>
          </div>
        ) : (
