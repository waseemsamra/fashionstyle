  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        let spotlight = productsArray.filter((p: any) => p.isDesignersSpotlight && p.name && p.name !== 'undefined');

        if (spotlight.length === 0) {
          spotlight = productsArray.filter((p: any) => p && p.isNew).slice(0, 20);
        }

        setProducts(spotlight.slice(0, 20));
      } catch (error) {
        console.error('Error loading designers spotlight:', error);
        setProducts([]);
      } finally {
        setIsLoading(false);
      }
    ...
    <section className="section-padding bg-white">
      <div className="container-custom">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-gold text-sm font-medium tracking-wider uppercase block mb-1">Spotlight</span>
            <h2 className="font-playfair text-3xl md:text-4xl font-semibold text-black">Designers Spotlight</h2>
          </div>
          <button
            onClick={() => navigate('/designers-spotlight')}
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Loading designers spotlight...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600">
              No spotlight products found yet. Please add products in Admin → Designers Spotlight.
            </p>
          </div>
        ) : (
