...
const loadData = async () => { try { setLoading(true); const data = await api.getAllProducts(); const productsArray = (data.items || []).filter((p: any) => p && p.id); setAllBrands([...new Set(productsArray.map((p: any) => p.brand).filter(Boolean))] as string[]); setProducts(productsArray); const collectionProducts = productsArray.filter((p: any) => p.isDesignersSpotlight); const selectedBrandSet = new Set(collectionProducts.map((p: any) => p.brand).filter(Boolean) as string[]); setSelectedBrands([...selectedBrandSet]); setSelectedProductIds(collectionProducts.map((p: any) => p.id)); } catch (error) { console.error('Failed to load data:', error); } finally { setLoading(false); } };
...
const handleSave = async () => { try { setSaving(true); localStorage.setItem('designersSpotlightProducts', JSON.stringify(selectedProductIds)); const token = localStorage.getItem('jwt_token'); if (!token) { alert('✅ Saved locally!'); return; } await Promise.all(products.map(async (product: any) => { const shouldFlag = selectedProductIds.includes(product.id); if (product.isDesignersSpotlight !== shouldFlag) return fetch(`${ADMIN_API_URL}/products/${product.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token.replace(/^["']|["']$/g, '')}` }, body: JSON.stringify({ ...product, isDesignersSpotlight: shouldFlag }) }); })); alert(`✅ Saved ${selectedProductIds.length} products!`); } catch (error) { alert('Failed to save'); } finally { setSaving(false); } };
