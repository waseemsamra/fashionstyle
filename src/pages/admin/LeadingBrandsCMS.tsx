...
export default function LeadingBrandsCMS() {
  ...
  const toggleProduct = (productId: string) => {
    setSelectedIds(prev => {
      ...
      if (prev.length >= MAX_LEADING) { alert(`You can only select up to ${MAX_LEADING} products`); return prev; }
  ...
  const handleSave = async () => {
    try {
      ...
      const updatePromises = allProducts.map(async (product) => {
        ...
        if (product.isLeadingBrands !== shouldFlag) {
          const response = await fetch(`${ADMIN_API_URL}/products/${product.id}`, {
            ...
            body: JSON.stringify({ ...product, isLeadingBrands: shouldFlag })
          });
          if (!response.ok) console.error('Failed to update product:', product.id);
          return response;
        }
      }).filter(Boolean);
    ...
    <div className="min-h-screen bg-beige-100 py-12">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-3xl font-bold mb-2">Leading Brands</h1><p className="text-gray-600">Select up to {MAX_LEADING} products ({allProducts.length} total)</p></div>
